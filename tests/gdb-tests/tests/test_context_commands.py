...
@pytest.mark.xfail
@pytest.mark.parametrize("cmd", ("k", "peb", "stepuntilasm", "bins", "tips", "setflag", "ai", "bc", "comm", "pwndbg", "brva", "ez", "lm", "be", "xpsr", "argv"))
def test_commands(start_binary, cmd):
    start_binary(USE_FDS_BINARY)  # it does not matter which binary we start here
    assert pwndbg.commands.context.context_disasm()
