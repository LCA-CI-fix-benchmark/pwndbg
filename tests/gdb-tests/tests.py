...
def parse_args():
    ...
    parser.add_argument(
        "--collect-only",
        action="store_true",
        help="only show the output of test collection without running any tests",
    )
    parser.add_argument(
        "test_name_filter", nargs="?", help="run only tests that match the regex", default=".*"
    )
    return parser.parse_args()cess

def get_tests_list(test_name_filter: str) -> List[str]:
...
def run_tests_and_print_stats(tests_list: List[str], args: argparse.Namespace) -> None:
    start = time.time()
    test_results: List[Tuple[CompletedProcess[str], str]] = []

    def handle_parallel_test_result(test_result: Tuple[CompletedProcess[str], str]) -> None:
        ...
        result = re.search(
            r"(\x1b\[3.m(PASSED|FAILED|SKIPPED|XPASS|XFAIL)\x1b\[0m)", content, re.MULTILINE).group(0)
    ...
    failed_tests = [(process, test) for (process, test) in test_results if process.returncode != 0]
    num_tests_failed = len(failed_tests)
    num_tests_passed_or_skipped = len(tests_list) - num_tests_failede
